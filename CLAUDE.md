...
### 브라우저 기본 팝업 금지
- `alert`, `confirm`, `prompt` 같은 브라우저 기본 팝업은 사용하지 않는다.
- 확인/경고/입력은 반드시 앱 UI 컴포넌트(`AlertDialog`, `Dialog`, `Toast` 등)로 구현한다.
...
- 토큰 비용 절감을 위해, 단순/반복 명령(예: 파일 조회, grep, 상태 확인)은 가능한 저비용 모델을 우선 사용한다.
- 설계 판단, 디버깅 추론, 리팩터링처럼 복잡한 작업에만 고성능 모델을 사용한다.
